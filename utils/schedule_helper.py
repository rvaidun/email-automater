...
def get_scheduled_send_time(
    csv_data: csv.DictReader, timezone: str
) -> bool | datetime.datetime:
    """
    Schedule an email based on current time and allowed ranges.

    Args:
        csv_data: CSV string with allowed time ranges
        timezone: Timezone to use for scheduling
    ...
    day_ranges = parse_time_ranges_csv(csv_data)
    ...
    return datetime.datetime.combine(now.date(), random_time, tzinfo=now.tzinfo)
